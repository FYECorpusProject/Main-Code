...
######################################################################
## first we will run through the data to get the last para numbers
## for each document
...
######################################################################
## The first thing we will do is compare adjacent sentences in the draft
movedHisto = defaultdict(int)
lineardict = defaultdict(list)
for key, value in sorted(theData.items()):
#    print('XX%sXX YY%sYY' % (key, value))

    keysplit = key.split()
    docname = keysplit[0]
    draftfinal = keysplit[1]
    draftsentnum = keysplit[2]
    finalsentnum = keysplit[3]

#    for item in keysplit:
#        print('ITEM ZZ%sZZ' % (item))

    if 'FINAL' == draftfinal: continue

    # create a dictionary
    # key is document name
    # value is a list of pairs of draft and final sent numbers
    # we skipped all the FINAL sents,
    #     so these should all be draft num aligned with final num
    thisList = lineardict[docname]
    thisList.append([draftsentnum, finalsentnum])
    lineardict[docname] = thisList
    
# at this point 'lineardict' is a dict from docname to list of
# [draft#, final#] sentence number pairs
# walk through, create the list of adjacencies, and then count
# those that are out of place
for key, value in sorted(lineardict.items()):
#    print('UU%sUU VV%sVV' % (key, value))

    # clear away the unaligned (i.e., deleted) draft sentences
    locallist = []
    for item in value:
        if '-99' != item[1]: # if draft sent was not deleted, use it
            locallist.append(item)
        else:
            fout.write('TRASH UNALIGNED DRAFT %s\n' % (item))

    # flag the sentences that are not in the "right" order
    for sub1 in range(0, len(locallist)-1):
        sub2 = sub1+1
        fout.write('SUBS %4d %4d %8s %8s\n' % (sub1, sub2, \
                                               locallist[sub1], locallist[sub2]))
        draft1 = int(locallist[sub1][0])
        final1 = int(locallist[sub1][1])
        draft2 = int(locallist[sub2][0])
        final2 = int(locallist[sub2][1])

        if draft1 >= draft2: # should never be true
            fout.write('ERROR %s (%4d %4d) (%4d %4d)\n' % \
                       (key, draft1, final1, draft2, final2))
            sys.exit()

        if final1 >= final2: # these are what we are looking for
            fout.write('INVERSION %s (%4d %4d) (%4d %4d)\n' % \
                       (key, draft1, final1, draft2, final2))

    # count the number of sentences whose aligned final is larger than
    # an aligned final that occurs further down
    countdowndict = defaultdict(int)
    countdowndict[key] = 0
    countGoingDown = 0
    for sub1 in range(0, len(locallist)-1):
        final1 = int(locallist[sub1][1])
        isOutOfPlace = False
        for sub2 in range(sub1+1, len(locallist)):
            final2 = int(locallist[sub2][1])
            if final1 > final2:
                isOutOfPlace = True
#                print('OUT OF PLACE %s : %4d %4d : %6d %6d' % \
#                         (key, sub1, sub2, final1, final2))
        if isOutOfPlace:
#            print('BUMP DOWN %s : %4d : %6d' % \
#                  (key, sub1, final1))
            countGoingDown += 1
            fout.write('COUNT DOWN %s : %4d : %6d\n' % (key, sub1, countGoingDown))
            countdowndict[key] = countGoingDown

    # count the number of sentences whose aligned final is smaller than
    # an aligned final that occurs further up
    countupdict = defaultdict(int)
    countupdict[key] = 0
    countGoingUp = 0
    for sub1 in range(len(locallist)-1, 0, -1):
        final1 = int(locallist[sub1][1])
        isOutOfPlace = False
        for sub2 in range(sub1-1, 0, -1):
            final2 = int(locallist[sub2][1])
            if final1 < final2:
                isOutOfPlace = True
#                print('OUT OF PLACE %s : %4d %4d : %6d %6d' % \
#                         (key, sub1, sub2, final1, final2))
        if isOutOfPlace:
#            print('BUMP UP %s : %4d : %6d' % \
#                  (key, sub1, final1))
            countGoingUp += 1
            fout.write('COUNT UP %s : %4d : %6d\n' % (key, sub1, countGoingUp))
            countupdict[key] = countGoingUp

    # we look for the smaller number of moves up compared to moves down as the
    # least cost movement to align final sentences in the same order. 
    movedDownCount = countdowndict[key]
    movedUpCount = countupdict[key]
    movedCount = min(movedDownCount, movedUpCount)

    movedHisto[movedCount] += 1

    fout.write('MOVED %s %4d\n' % (key, movedCount))

for key, value in sorted(movedHisto.items()):
    fout.write('MOVEDHISTO %4s %4d\n' % (key, value))
